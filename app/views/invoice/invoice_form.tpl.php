...
<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="invoice[duedate]" class="form-control date" value="<?php echo date_format(date_create($result['duedate']), 'd-m-Y'); ?>" placeholder="<?php echo $lang['invoices']['text_due_date']; ?>">
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <select name="invoice[status]" id="" class="custom-select" required>
                                <option value="Paid" <?php if ("paid" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_paid']; ?></option>
                                <option value="Partially Paid" <?php if ("Partially Paid" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_partially_paid']; ?></option>
                                <option value="Unpaid" <?php if ("Unpaid" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_unpaid']; ?></option>
                                <option value="Pending" <?php if ("Pending" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_pending']; ?></option>
                                <option value="In Process" <?php if ("In Process" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_in_process']; ?></option>
                                <option value="Cancelled" <?php if ("Cancelled" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_cancelled']; ?></option>
                                <option value="Other" <?php if ($lang['invoices']['text_other'] == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_other']; ?></option>
                                <option value="Unknown" <?php if ("Unknown" == $result['status']) { echo "selected"; } ?>><?php echo $lang['invoices']['text_unknown']; ?></option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="invoice-items pt-3 pb-5">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="invoice[note]" rows="3"><?php if (!empty($result['id'])) { echo $result['note']; } else { echo $info['invoice_setting']['customernote']; } ?></textarea>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
                        <label><?php echo $lang['invoices']['text_terms_Conditions']; ?></label>
                        <textarea class="form-control" name="invoice[tc]" rows="3"><?php if (!empty($result['id'])) { echo $result['tc']; } else { echo $info['invoice_setting']['tc']; } ?></textarea>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label><?php echo $lang['invoices']['text_want_payment_info_on_invoice']; ?></label>
                        <div>
                            <div class="custom-control custom-radio custom-radio-1 d-inline-block">
                                <input type="radio" name="invoice[want_payment]" class="custom-control-input" value="1" id="paymentinfo-yes" <?php if ($result['want_payment'] == '1') { echo "checked"; } ?>>
                                <label class="custom-control-label" for="paymentinfo-yes"><?php echo $lang['invoices']['text_yes']; ?></label>
                            </div>
                            <div class="custom-control custom-radio custom-radio-1 d-inline-block">
                                <input type="radio" name="invoice[want_payment]" class="custom-control-input" value="0" id="paymentinfo-no" <?php if ($result['want_payment'] == '0') { echo "checked"; } ?>>
                                <label class="custom-control-label" for="paymentinfo-no"><?php echo $lang['invoices']['text_no']; ?></label>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label><?php echo $lang['invoices']['text_want_signature_on_invoice']; ?></label>
                        <div>
                            <div class="custom-control custom-radio custom-radio-1 d-inline-block">
                                <input type="radio" name="invoice[want_signature]" class="custom-control-input" value="1" id="signature-yes" <?php if ($result['want_signature'] == '1') { echo "checked"; } ?>>
                                <label class="custom-control-label" for="signature-yes"><?php echo $lang['invoices']['text_yes']; ?></label>
                            </div>
                            <div class="custom-control custom-radio custom-radio-1 d-inline-block">
                                <input type="radio" name="invoice[want_signature]" class="custom-control-input" value="0" id="signature-no" <?php if ($result['want_signature'] == '0') { echo "checked"; } ?>>
                                <label class="custom-control-label" for="signature-no"><?php echo $lang['invoices']['text_no']; ?></label>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-gradient btn-pill add-modal-taxes"><?php echo $lang['invoices']['text_add_taxes']; ?></button>
            </div>
        </div>
    </div>
</div>
