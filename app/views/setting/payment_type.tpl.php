            ...
            <div class="panel panel-default">
                <div class="panel-head">
                    ...
                    <div class="panel-action">
                        <a class="btn btn-primary btn-outline btn-gradient btn-pill btn-sm" data-toggle="modal" data-target="#addPaymentTypeModel"><i class="icon-plus mr-1"></i> <?php echo $lang['settings']['text_new_payment_method']; ?></a>
                    </div>  
                </div>
                        ...
                        <table class="table table-bordered table-striped datatable-table" width="100%">
                            <thead>
                                <tr class="table-heading">
                                    ...
                                    <th><?php echo $lang['settings']['text_payment_method']; ?></th>
                            ...
                            <tbody>
                                ...
                                <tr> 
                                    ...
                                    <td class="table-action">
                                        <a class="btn btn-primary btn-circle btn-outline btn-outline-1x edit-paymenttype" data-toggle="tooltip" title="<?php echo $lang['common']['text_edit']; ?>" data-name="<?php echo $value['name'] ?>" data-description="<?php echo $value['description'] ?>" data-id="<?php echo $value['id'] ?>" data-status="<?php echo $value['status'] ?>">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="<?php echo URL.DIR_ROUTE.'paymenttype/delete'; ?>" class="delete-card-button" method="post">
...
<!-- ADD EDIT MODAL -->
<div class="modal fade" id="addPaymentTypeModel" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php echo $lang['settings']['text_new_payment_method']; ?></h5>
            ...
            <form action="<?php echo $action; ?>" method="post">
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-form-label"><?php echo $lang['settings']['text_payment_method_name']; ?></label>
                        <input type="text" class="form-control" name="name" placeholder="<?php echo $lang['settings']['text_payment_method_name']; ?>">
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function () {
        ...
        $('body').on('click', '.edit-paymenttype', function () {
            var ele = $(this);
            $('#addPaymentTypeModel input[name="name"]').val(ele.data("name"));
            $('#addPaymentTypeModel textarea[name="description"]').val(ele.data("description"));
            $('#addPaymentTypeModel input[name="id"]').val(ele.data("id"));
            $('#addPaymentTypeModel select[name="status"]').val(ele.data("status"));
            $('#addPaymentTypeModel .modal-title').text('<?php echo $lang['settings']['text_edit_payment_method']; ?>');
            $('#addPaymentTypeModel').modal('show');
        });

        $('#addPaymentTypeModel').on('hidden.bs.modal', function (e) {
            $('#addPaymentTypeModel .modal-title').text('<?php echo $lang['settings']['text_new_payment_method']; ?>');
            $('#addPaymentTypeModel input').not( "[name='_token']" ).val('');
            $('#addPaymentTypeModel textarea').val('');
        });
        $('#finance-method').addClass('active');
