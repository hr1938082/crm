            ...
            <div class="panel panel-default">
                <div class="panel-head">
                    ...
                    <div class="panel-action">
                        <a class="btn btn-primary btn-outline btn-gradient btn-pill btn-sm" data-toggle="modal" data-target="#addCurrencyModel"><i class="icon-plus mr-1"></i> <?php echo $lang['settings']['text_new_currency']; ?></a>
                    </div>  
                </div>
                        ...
                        <table class="table table-bordered table-striped datatable-table" width="100%">
                            <thead>
                                <tr class="table-heading">
                                    ...
                                    <th><?php echo $lang['settings']['text_currency_name']; ?></th>
                                    <th><?php echo $lang['settings']['text_abbr']; ?></th>
                            ...
                            <tbody>
                                ...
                                <tr> 
                                    ...
                                    <td class="table-action">
                                        <a class="btn btn-primary btn-circle btn-outline btn-outline-1x edit-currency" data-toggle="tooltip" title="<?php echo $lang['common']['text_edit']; ?>" data-name="<?php echo $value['name'] ?>" data-abbr="<?php echo $value['abbr'] ?>" data-id="<?php echo $value['id'] ?>" data-status="<?php echo $value['status'] ?>">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="<?php echo URL.DIR_ROUTE.'currency/delete'; ?>" class="delete-card-button" method="post">
...
<div class="modal fade" id="addCurrencyModel" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php echo $lang['settings']['text_new_currency']; ?></h5>
            ...
            <form action="<?php echo $action; ?>" method="post">
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-form-label"><?php echo $lang['settings']['text_currency_name']; ?></label>
                        <input type="text" class="form-control" name="name" placeholder="<?php echo $lang['settings']['text_currency_name']; ?>" required>
                    </div>
                    <div class="form-group">
                        <label class="col-form-label"><?php echo $lang['settings']['text_abbr']; ?></label>
                        <input type="text" class="form-control" name="abbr" placeholder="<?php echo $lang['settings']['text_abbr']; ?>" required>
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function () {
        ...
        $('body').on('click', '.edit-currency', function () {
            ...
            $('#addCurrencyModel .modal-title').text('<?php echo $lang['settings']['text_edit_currency']; ?>');
            $('#addCurrencyModel').modal('show');
        });

        $('#addCurrencyModel').on('hidden.bs.modal', function (e) {
            $('#addCurrencyModel .modal-title').text('<?php echo $lang['settings']['text_new_currency']; ?>');
            $('#addCurrencyModel input').not( "[name='_token']" ).val('');
            $('#addCurrencyModel textarea').val('');
        });

        $('#finance-currency').addClass('active');
    });
</script>
<!-- Footer -->
